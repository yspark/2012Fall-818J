...
10 movies are selected from \term{Popular on Netflix} section provided by Netflix website and the list of selected movices are shown in Table \ref{tab:movies}.
...
After building up test data set and training data set, we apply KNN method in order to classify each test data based on training data set. 
...
The accuracy is relatively low as 44% when subsequence length is set to 60-second.
However, the accuracy increases up to 88% when subsequence length is increased to 180-second.
